...
class NFeToolkit(object):
    ...
    @staticmethod
    def __create_xml_folders(path: str):
        """Cria as pastas necessárias para armazenar os arquivos XML."""    
        if not os.path.exists(path):
            os.makedirs(path)

        for folder in ['nfe', 'canc', 'cce', 'inut']:
            if not os.path.exists(f"{path}\\{folder}"):
                os.makedirs(f"{path}\\{folder}")

    def organize_xmls(self, src_dir_fd: str, dest_dir_fd: str):
        """oraniza os arquivos xml contidos em uma pasta e os move para subpastas de 
        um diretório fornecido pelo usuário (subpastas: nfe, canc, cce e inut)"""
        self.__create_xml_folders(dest_dir_fd)
                
        for root, dirs, files in os.walk(src_dir_fd):
            for file in files:
                ...
                if file.endswith('.zip'):
                    ...
                    try:
                        dest_folder = self.xml_type(file_path)
                        if dest_folder == 'undefined':
                            print(f"Arquivo {file} não é um arquivo xml conhecido")
                        else:
                            file_path.rename(Path(dest_dir_fd) / dest_folder / file)
    ...
    def _list_xml(self, src_dir_fd: str):
        return [
    ...
    def store_nfe_from_dir(self, src_dir_fd: str, rep: RepositorioNFe):
        
        xml_list = self._list_xml(src_dir_fd)
        for xml_file in tqdm.tqdm(xml_list, total=len(xml_list), desc="processando xmls"):            
            ...
            if xml_type == 'nfe':
                ...
                self._store_nfe(rep, obj)
            elif xml_type == 'canc':
                obj = self.evento_canc_from_path(xml_file)
                self._store_evt(rep, obj)
            elif xml_type == 'cce':      
                obj = self.evento_cce_from_path(xml_file)
                self._store_evt(rep, obj)         
                
    def _store_evt(self, rep: RepositorioNFe, evt: Any):
    ...
    def _store_nfe(self, rep: RepositorioNFe, nfeProc: NfeProc):
        ...
        if nfeProc.NFe.infNFe.cobr:
            fat = nfeProc.NFe.infNFe.cobr.fat
            if fat:
                n140 = RegistroN140() 
